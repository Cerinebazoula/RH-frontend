  ...
  return (
    <>
      <div className="ml-5">
        ...
        <p className="text-base font-normal leading-relaxed text-gray-600 row-start-2">You can add , delete or modify any of the members</p>
        <div className="flex justify-between items-center w-full mt-8">
          <div className="flex gap-6 w-fit h-fit">
            <AddButton target="member" onClick={() => setShowAddProfileModel(!showAddProfileModel)} />
            {Boolean(profileClicked) && <EditButton target="member" onClick={() => setShowEditProfileModel(!showEditProfileModel)} />}
          </div>
          <SearchBar
      ...
      <div className="h-full w-full flex mt-4 pb-48">
        <Profiles profiles={isSearching ? filteredProfiles : profiles} onClick={handleClickProfileEvent} />
        {Boolean(profileClicked) && <DetailedProfile
      ...
      <PopUpModel
        ...
        onClose={() => { setShowAddProfileModel(false); }}
        action="Add"
        target="member"
      />
      <PopUpModel
        visible={showEditProfileModel}
        onClose={() => { setShowEditProfileModel(false); }}
        action="Edit"
        target="member"
      />
    </>
